...
/* 用户需要查看文件系统信息时，会通过 VFS 调用次方法 ,kstatfs 结构主要定义在
 * linux/statfs.h 中*/
...
struct dentry *simple_lookup(struct inode *dir, struct dentry *dentry,
  ...
  if (!dentry->d_sb->s_d_op) d_set_d_op(dentry, &simple_dentry_operations);
  d_add(dentry, NULL);
  return NULL;
}
...
static struct dentry *scan_positives(struct dentry *cursor, struct list_head *p,
  ...
  while ((p = p->next) != &dentry->d_subdirs) {
    ...
    if (d->d_flags & DCACHE_DENTRY_CURSOR) continue;
    if (simple_positive(d) && !--count) {
      spin_lock_nested(&d->d_lock, DENTRY_D_LOCK_NESTED);
      if (simple_positive(d)) found = dget_dlock(d);
      spin_unlock(&d->d_lock);
      if (likely(found)) break;
      count = 1;
    }
    if (need_resched()) {
...
loff_t dcache_dir_lseek(struct file *file, loff_t offset, int whence) {
  ...
  if (offset != file->f_pos) {
    ...
    if (offset > 2)
      to = scan_positives(cursor, &dentry->d_subdirs, offset - 2, NULL);
    spin_lock(&dentry->d_lock);
    if (to)
...
static struct dentry *find_next_child(struct dentry *parent,
  ...
  while ((p = p->next) != &parent->d_subdirs) {
    ...
    if (simple_positive(d)) {
      spin_lock_nested(&d->d_lock, DENTRY_D_LOCK_NESTED);
      if (simple_positive(d)) child = dget_dlock(d);
      spin_unlock(&d->d_lock);
      if (likely(child)) break;
    }
  }
...
void simple_recursive_removal(struct dentry *dentry,
  ...
  while (true) {
    ...
    inode_lock(inode);
    if (d_is_dir(this)) inode->i_flags |= S_DEAD;
    while ((child = find_next_child(this, victim)) == NULL) {
      ...
      if (simple_positive(victim)) {
        ...
        if (callback) callback(victim);
        dput(victim);  // unpin it
      }
      if (victim == dentry) {
        inode->i_ctime = inode->i_mtime = current_time(inode);
        if (d_is_dir(dentry)) drop_nlink(inode);
        inode_unlock(inode);
        dput(dentry);
        return;
      }
...
static void pseudo_fs_free(struct fs_context *fc) { kfree(fc->fs_private); }
...
int simple_rmdir(struct inode *dir, struct dentry *dentry) {
  if (!simple_empty(dentry)) return -ENOTEMPTY;
...
int simple_write_end(struct file *file, struct address_space *mapping,
   ...
   */
  if (last_pos > inode->i_size) i_size_write(inode, last_pos);
...
ssize_t simple_attr_read(struct file *file, char __user *buf, size_t len,
  ...
  if (*ppos && attr->get_buf[0]) {
    ...
    size = scnprintf(attr->get_buf, sizeof(attr->get_buf), attr->fmt,
...
ssize_t simple_attr_write(struct file *file, const char __user *buf, size_t len,
  ...
  if (copy_from_user(attr->set_buf, buf, size)) goto out;

  attr->set_buf[size] = '\0';
  ret = kstrtoull(attr->set_buf, 0, &val);
  if (ret) goto out;
  ret = attr->set(attr->data, val);
  if (ret == 0) ret = len; /* on success, claim we got the whole input */
out:
  mutex_unlock(&attr->mutex);
  return ret;
}
...
struct dentry *generic_fh_to_dentry(
    struct super_block *sb, struct fid *fid, int fh_len, int fh_type,
...
int generic_check_addressable(unsigned blocksize_bits, u64 num_blocks) {
  ...
  u64 last_fs_page = last_fs_block >> (PAGE_SHIFT - blocksize_bits);

  if (unlikely(num_blocks == 0)) return 0;

  if ((blocksize_bits < 9) || (blocksize_bits > PAGE_SHIFT)) return -EINVAL;
 ...
 * anon inodes.
 */
static int anon_set_page_dirty(struct page *page) { return 0; };
 ...
 */
int simple_nosetlease(struct file *filp, long arg, struct file_lock **flp,
                      void **priv) {
  return -EINVAL;
}
...
static int generic_ci_d_compare(const struct dentry *dentry, unsigned int len,
  ...
  if (!dir || !needs_casefold(dir)) goto fallback;
  /*
...
static int generic_ci_d_hash(const struct dentry *dentry, struct qstr *str) {
  ...
  if (!dir || !needs_casefold(dir)) return 0;

  ret = utf8_casefold_hash(um, dentry, str);
  if (ret < 0 && sb_has_strict_encoding(sb)) return -EINVAL;
  return 0;
}
