...
struct ext2_sb_info {
	...
	unsigned long
		s_itb_per_group; /* Number of inode table blocks per group */
	unsigned long s_gdb_count; /* Number of group descriptor blocks */
	unsigned long
		s_desc_per_block; /* Number of group descriptors per block */
...
#define ext2_debug(f, a...)                                               \
	{                                                                 \
		printk("EXT2-fs DEBUG (%s, %d): %s:", __FILE__, __LINE__, \
		       __func__);                                         \
		printk(f, ##a);                                           \
	}
#else
...
/*
 * Structure of a blocks group descriptor,整体大小 32 bytes。紧跟在 super block
 * 之后，如果初始化文件系统后存在多个 block group,每一个 block group 都需要一个
 * group desc.
 */
...
#define EXT2_DIRSYNC_FL \
	FS_DIRSYNC_FL /* dirsync behaviour (directories only) \
                                     */
...
#define EXT2_FL_USER_MODIFIABLE \
	FS_FL_USER_MODIFIABLE /* User modifiable flags */
...
/**
 * Structure of an inode on the disk,default: 128 bytes
 */
struct ext2_inode {
	...
	/* Pointers to blocks ,EXT2_N_BLOCKS(default）:15,保存指向对应 block 的指针*/
 ...
 * file_type field.
 * 新版本的 ext2 文件系统目录项的数据结构，通过这种结构能够建立这样一种映射关系：
 * 目录下的文件名称 < === > 目录下此文件对应的 InodeId
 */
struct ext2_dir_entry_2 {
	__le32 inode; /* Inode number */
	/* `rec_len` & `name_len`是辅助我们实现目录项遍历和管理的。  */
	__le16 rec_len; /* Directory entry length */
...
/* 整个 ext2 文件系统将会使用的 API 原型以及对应的文件位置 */
...
extern int ext2_inode_by_name(struct inode *dir, const struct qstr *child,
			      ino_t *ino);
extern int ext2_make_empty(struct inode *, struct inode *);
extern struct ext2_dir_entry_2 *ext2_find_entry(struct inode *,
						const struct qstr *,
						struct page **,
						void **res_page_addr);
...
/* super.c */
extern __printf(3, 4) void ext2_error(struct super_block *, const char *,
				      const char *, ...);
extern __printf(3, 4) void ext2_msg(struct super_block *, const char *,
				    const char *, ...);
