...
static int __init init_inodecache(void)
{
	ext2_inode_cachep = kmem_cache_create_usercopy(
		...
		sizeof_field(struct ext2_inode_info, i_data), init_once);
...
static struct inode *ext2_nfs_get_inode(struct super_block *sb, u64 ino,
					u32 generation)
{
	struct inode *inode;
...
static const match_table_t tokens = { { Opt_bsd_df, "bsddf" },
			    ...
			    int read_only)
{
	...
	if (!(sbi->s_mount_state & EXT2_VALID_FS))
		...
		ext2_msg(sb, KERN_WARNING,
		 ...
		 le16_to_cpu(es->s_mnt_count) >=
			 (unsigned short)(__s16)le16_to_cpu(
				 es->s_max_mnt_count))
		ext2_msg(sb, KERN_WARNING,
	...
	if (test_opt(sb, DEBUG))
		ext2_msg(sb, KERN_INFO,
			 ...
			 sbi->s_frag_size, sbi->s_groups_count,
			 EXT2_BLOCKS_PER_GROUP(sb), EXT2_INODES_PER_GROUP(sb),
			 sbi->s_mount_opt);
	return res;
}
...
static int ext2_fill_super(struct super_block *sb, void *data, int silent)
{
   ...
   */
	...
	if (!sb_rdonly(sb) && (features = EXT2_HAS_RO_COMPAT_FEATURE(
	...
	if (sb->s_blocksize != blocksize) {
		...
		if (!sb_set_blocksize(sb, blocksize)) {
			ext2_msg(sb, KERN_ERR, "error: bad blocksize %d",
				 blocksize);
			goto failed_sbi;
		}
	...
	sbi->s_group_desc = kmalloc_array(
		db_count, sizeof(struct buffer_head *), GFP_KERNEL);
...
static int ext2_remount(struct super_block *sb, int *flags, char *data)
{
     ...
     */
		...
		__le32 ret = EXT2_HAS_RO_COMPAT_FEATURE(
			sb, ~EXT2_FEATURE_RO_COMPAT_SUPP);
		if (ret) {
			spin_unlock(&sbi->s_lock);
			ext2_msg(sb, KERN_WARNING,
