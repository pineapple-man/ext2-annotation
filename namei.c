...
static struct dentry *ext2_lookup(struct inode *dir, struct dentry *dentry,
  ...
  if (dentry->d_name.len > EXT2_NAME_LEN) return ERR_PTR(-ENAMETOOLONG);
 ...
 * with d_instantiate().
 */
static int ext2_create(struct user_namespace *mnt_userns, struct inode *dir,
                       struct dentry *dentry, umode_t mode, bool excl) {
  /**
   * create function performs the following operations:
   * 1. Introduces a new entry into the physical structure on the disk; the
   *update of the bit maps on thd disk must not be forgotten.
   * 2. Configures access rights to those received as a parameter.
   * 3. Marks the inode as dirty with the `mark_inode_dirty()` function.
   * 4. Instantiates the directory entry(dentry) with the `d_instantiate()`
   *function*/
  struct inode *inode;
  int err;
...
static int ext2_symlink(struct user_namespace *mnt_userns, struct inode *dir,
   ...
   * 1. Binds the new dentry to the inode.
   * 2. Increments the `i_nlink` field of the inode.
   * 3. Marks the inode as dirty using the `mark_inode_dirty()` function
   */
...
static int ext2_mkdir(struct user_namespace *mnt_userns, struct inode *dir,
                      struct dentry *dentry, umode_t mode) {
  /**
   * ext2_mkdir function performs the following operations:
   * 1. Call ext2_create();
   * 2. Allocateds a data block for the directory.
   * 3. Create the `.` and `..` entries
   * */
  struct inode *inode;
  int err;
...
static int ext2_rename(struct user_namespace *mnt_userns, struct inode *old_dir,
  ...
  old_de =
      ext2_find_entry(old_dir, &old_dentry->d_name, &old_page, &old_page_addr);
  if (IS_ERR(old_de)) {
    err = PTR_ERR(old_de);
    goto out;
  }
  ...
  if (new_inode) {
    ...
    err = -ENOTEMPTY;
    if (dir_de && !ext2_empty_dir(new_inode)) goto out_dir;

    new_de =
        ext2_find_entry(new_dir, &new_dentry->d_name, &new_page, &page_addr);
    if (IS_ERR(new_de)) {
      err = PTR_ERR(new_de);
      goto out_dir;
    }
    ...
    if (err) goto out_dir;
    if (dir_de) inode_inc_link_count(new_dir);
  }
  ...
  if (dir_de) {
    if (old_dir != new_dir)
      ext2_set_link(old_inode, dir_de, dir_page, dir_page_addr, new_dir, 0);
...
out_dir:
  if (dir_de) ext2_put_page(dir_page, dir_page_addr);
out_old:
  ext2_put_page(old_page, old_page_addr);
out:
  return err;
}
/* ext2 文件系统中对 dir 类型的 inode 操作 */
